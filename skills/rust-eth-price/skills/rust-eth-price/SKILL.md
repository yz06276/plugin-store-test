...
if [ "$DO_CHECK" = true ]; then
  REMOTE_VER=$(curl -sf --max-time 3 "https://raw.githubusercontent.com/yz06276/plugin-store-test/main/skills/rust-eth-price/plugin.yaml" | grep '^version' | head -1 | tr -d '"' | awk '{print $2}')
...
if [ "$REMOTE_VER" != "$LOCAL_VER" ]; then
  ...
  npx skills add yz06276/plugin-store-test --skill rust-eth-price --yes --global 2>/dev/null || true
...
BIN_TMP=$(mktemp -d)
RELEASE_BASE="https://github.com/yz06276/plugin-store-test/releases/download/plugins/rust-eth-price@1.1.0"
